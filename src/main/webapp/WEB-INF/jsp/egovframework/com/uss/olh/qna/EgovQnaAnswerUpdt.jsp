...
function fn_egov_updt_qna(form){
	if (!validateQnaVO(form)) {		 			
		...
		if(confirm("<spring:message code="common.update.msg" />")){
			//form.submit();
	        $("#qnaVO").ajaxForm({
	        	url:"${pageContext.request.contextPath}/uss/olh/qna/updateQnaAnswer.do",
...
<!-- 상단타이틀 -->
<form:form commandName="qnaVO" name="qnaVO" action="${pageContext.request.contextPath}/uss/olh/qna/updateQnaAnswer.do" method="post" onSubmit="fn_egov_updt_qna(document.forms[0]); return false;">  
<div class="wTableFrm">
	<h1><img src="/images/bl/bl_circle.gif" class="img_margin1"> ${pageTitle} <spring:message code="title.create" /></h1>
	...
	<!-- 하단 버튼 -->
	<div class="btn">
		<input type="button" class="s_submit" onClick="fn_egov_updt_qna(document.qnaVO); return false;" value="<spring:message code="title.create" />" title="<spring:message code="title.create" /> <spring:message code="input.button" />" />
		<input type="button" class="s_submit" onClick="fnList(); return false;"  value="<spring:message code="button.list" />" title="<spring:message code="title.list" /> <spring:message code="input.button" />" />		
