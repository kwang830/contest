...
function fnUpdate(form){	
    if(validatePasswordChgVO(form)){
    	...
    	$.ajax({
    		type:"POST",
    		url:"${pageContext.request.contextPath}/uss/umt/EgovUserPasswordUpdt.do",
    		data: string, 
    		dataType:'html',
    		timeout:(1000*30),
...
<form id="form1" name="passwordChgVO" method="post"  action="<c:url value="${'/uss/umt/EgovUserPasswordUpdt.do'}"/>" onsubmit="fnUpdate(document.forms[0]); return false;">
...
<div class="wTableFrm">
 	<!-- 타이틀 -->
	<h1><img src="/images/bl/bl_circle.gif" class="img_margin1"> <spring:message code="comUssUmt.deptUserManage.title" /> ${pageTitle}</h1>
	...
	<tbody>
		...
		<tr>
			<th>${title}</th>
			<td class="left">
				<input name="emplyrId" id="emplyrId" title="사용자아이디" readonly="readonly" type="text" size="20" value="<c:out value='${userManageVO.emplyrId}'/>"  maxlength="20" readonly/>
