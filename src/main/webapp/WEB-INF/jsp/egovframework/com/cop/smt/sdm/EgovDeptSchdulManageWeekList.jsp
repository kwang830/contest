...
<HEAD>
	...
	******************************************************** */
	...
	function fnEgovSchdulSe(setValue) {
		$.ajax({
			type:"POST",
			url:"${pageContext.request.contextPath}/cop/smt/sdm/EgovDeptSchdulManageWeekList.do",
			data:{
				"year": "<%=iNowYear%>",
				"month": "<%=iNowMonth%>",
				"week": "<%=iNowWeek%>",
	...
	function fnEgovSchdulSrc(year,month,week) {
		$.ajax({
			type:"POST",
			url:"${pageContext.request.contextPath}/cop/smt/sdm/EgovDeptSchdulManageWeekList.do",
			data:{
...
<div class="sort_area">
	...
	<div class="date_view">
		<ul>
			...
			<li><a class="prev" onClick="fnEgovSchdulSrc('<%=iNowYear-1%>','<%=iNowMonth%>','<%=iNowWeek%>');" href="javascript:;">prev</a></li>
			<li class="date"><%=iNowYear%> <spring:message code="comCopSmtSdm.Navi.Year"/></li>
			<li><a class="next" onClick="fnEgovSchdulSrc('<%=iNowYear+1%>','<%=iNowMonth%>','<%=iNowWeek%>');" href="javascript:;">next</a></li>
			<li class="date">&nbsp;</li>
			<%if(iNowMonth > 0 ){ %>
			<li><a class="prev" onClick="fnEgovSchdulSrc('<%=iNowYear%>','<%=iNowMonth-1%>','<%=iNowWeek%>');" href="javascript:;">prev</a></li>
			<%}%>
			<li class="date"><%=iNowMonth+1%> <spring:message code="comCopSmtSdm.Navi.Month"/></li><!-- 월 -->
			<%if(iNowMonth < 11 ){ %>
			<li><a class="next" onClick="fnEgovSchdulSrc('<%=iNowYear%>','<%=iNowMonth+1%>','<%=iNowWeek%>');" href="javascript:;">next</a></li>
			<%}%>
			<li class="date">&nbsp;</li>
			<%if(iNowWeek > 0 ){ %>
			<li><a class="prev" onClick="fnEgovSchdulSrc('<%=iNowYear%>','<%=iNowMonth%>','<%=iNowWeek-1%>');" href="javascript:;">prev</a></li>
			<%}%>
			<li class="date"><%=iNowWeek+1%> <spring:message code="comCopSmtSdm.Navi.Week"/> </li><!-- 주 -->
			<%if(iNowWeek < listWeekGrop.size()-1 ){ %>
			<li><a class="next" onClick="fnEgovSchdulSrc('<%=iNowYear%>','<%=iNowMonth%>','<%=iNowWeek+1%>');" href="javascript:;">next</a></li>
			<%}%>
