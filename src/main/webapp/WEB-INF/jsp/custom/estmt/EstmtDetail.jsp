 ...
 * 모달셋팅
 ******************************************************** */
function fn_modal3_setting(){

	//버튼에 모달 연결
	$("#btn_excel").egovModal( "egovModal3" );
	
	//타이틀 설졍
	$("#egovModal3").setEgovModalTitle("엑셀다운로드");
	
	var contentAddTable = "";
	contentAddTable += "<table class='board_list' id='bcncTableList'>";
	contentAddTable += "<caption>거래처 검색 정보</caption>";
	contentAddTable += "<colgroup>";
	contentAddTable += "<col style='width: 50%;'>";
	contentAddTable += "<col style='width: 50%;'>";
	contentAddTable += "</colgroup>";
	contentAddTable += "<thead>";
	contentAddTable += "<tr style='height:33px;'>";
	contentAddTable += "<th>견적서 국내용</th>";
	contentAddTable += "<th>견적서 수출용</th>";
	contentAddTable += "</tr>";
	contentAddTable += "<tr style='height:33px;'>";
	contentAddTable += "<th><span class='btn_style1 blue' id='btnExcelDown1' onclick='fn_goExcel1()'><a href='#'>국문양식</a></span></th>";
	contentAddTable += "<th><span class='btn_style1 blue' id='btnExcelDown1' onclick='fn_goExcel2()'><a href='#'>영문양식</a></span></th>";
	contentAddTable += "</tr>";
	contentAddTable += "<tr style='height:33px;'>";
	contentAddTable += "<th>&nbsp;</th>";
	contentAddTable += "<th><span class='btn_style1 blue' id='btnExcelDown2' onclick='fn_goExcel3()'><a href='#'>상세리스트</a></span></th>";
	contentAddTable += "</tr>";	
	contentAddTable += "</thead>";
	contentAddTable += "<tbody class='ov'>";
	contentAddTable += "</tbody>";
...
/* 	content += "<div class='modal-alignL' style='margin:5px 0 0 0'>"+"거래처 명 :"+"</div>"; //사용할아이디
	...
	//모달 body 설정
	$("#egovModal3").setEgovModalBody(content);

	var footer = "";
	//footer += "<span class='btn_style1 blue' id='btnModalSelect' onclick='fn_bcnc_check()'><a href='#'>조회</a></span>&nbsp;";
	
	//모달 footer 설정
	$("#egovModal3").setEgovModalfooter(footer);
...
function fn_goExcel1(){
	var manageNo = $("#manageNo").val();
	window.open("/custom/estmt/downExcel.do?manageNo="+manageNo+"&xlsType=1");
}
function fn_goExcel2(){
	var manageNo = $("#manageNo").val();
	window.open("/custom/estmt/downExcel.do?manageNo="+manageNo+"&xlsType=2");
}
function fn_goExcel3(){
	var manageNo = $("#manageNo").val();
	window.open("/custom/estmt/downExcel.do?manageNo="+manageNo+"&xlsType=3");
}
$(function(){
...
<table class="board_list" summary="">
	...
	<tr>
		<th>문서번호</th>
		<th><input class="s_input" name="docNo" id="docNo" type="text"  size="25" title="" value="<c:out value="${estmtManageVO.docNo}"/>"  maxlength="20" style="width:100%;" readonly="readonly"></th>
		<th>등록일자</th>
		<th><input class="s_input" name="registDe" id="registDe" type="text"  size="25" title="" value="<c:out value="${estmtManageVO.registDe}"/>"  maxlength="20" readonly="readonly"></th>
		<th>계약일자</th>
		<th><input class="s_input" name="cntrctDe" id="cntrctDe" type="text"  size="25" title="" value="<c:out value="${estmtManageVO.cntrctDe}"/>"  maxlength="20" readonly="readonly"></th>
		<th>진행상태</th>
		<th>
			<form:select path="progrsSttus" id="progrsSttus" title="진행상태" disabled="true">
	...
	<tr>
		<th>문서제목</th>
		<th colspan="7"><input class="s_input" name="docSj" id="docSj" type="text"  size="25" title="" value="<c:out value="${estmtManageVO.docSj}"/>"  maxlength="100" style="width:100%;" readonly="readonly"></th>
	</tr>	
	</thead>
	<tbody class="ov">
	</tbody>
...
<table class="board_list" summary="" id="estmtList">
	...
	<tr>
		...
		<th>거래처담당자</th>
		<th><input class="s_input" name="bcncCharger" type="text"  size="25" title="" value=""  maxlength="20" style="width:100%;" readonly="readonly"></th>
		<th>연락처</th>
		<th><input class="s_input" name="cttpc" type="text"  size="25" title="" value=""  maxlength="20" style="width:100%;" readonly="readonly"></th>
		<th>이메일</th>
		<th><input class="s_input" name="email" type="text"  size="25" title="" value=""  maxlength="20" style="width:100%;" readonly="readonly"></th>
		<th>업무담당자</th>
		<th><input class="s_input" name="jobCharger" type="text"  size="25" title="" value=""  maxlength="20" style="width:100%;" readonly="readonly"></th>
		<th>대표정보</th>
	...
	<tr>
		...
		<th>메모</th>
		<th colspan="3"><input class="s_input" name="memo" type="text"  size="25" title="" value="" maxlength="20" style="width:100%;" readonly="readonly"></th>
	</tr>
		</c:when>
		<c:otherwise>
	...
	<tr>
		<th>거래처담당자</th>
		<th><input type="hidden" name="subSn" value="<c:out value="${resultInfo.subSn}"/>" readonly="readonly"><input class="s_input" name="bcncCharger" type="text"  size="25" title="" value="<c:out value="${resultInfo.bcncCharger}"/>"  maxlength="20" style="width:100%;" readonly="readonly"></th>
		<th>연락처</th>
		<th><input class="s_input" name="cttpc" type="text"  size="25" title="" value="<c:out value="${resultInfo.cttpc}"/>"  maxlength="20" style="width:100%;" readonly="readonly"></th>
		<th>이메일</th>
		<th><input class="s_input" name="email" type="text"  size="25" title="" value="<c:out value="${resultInfo.email}"/>"  maxlength="20" style="width:100%;" readonly="readonly"></th>
		<th>업무담당자</th>
		<th><input class="s_input" name="jobCharger" type="text"  size="25" title="" value="<c:out value="${resultInfo.jobCharger}"/>"  maxlength="20" style="width:100%;" readonly="readonly"></th>
		<th>대표정보</th>
		<th><input type="radio" name="mainYnChk" id="mainYnChk" <c:if test="${resultInfo.mainYn == '1'}">checked="checked"</c:if>><input type="hidden" name="mainYn" id="mainYn" value="<c:out value="${resultInfo.mainYn}"/>" readonly="readonly"></th>		
	</tr>
	<tr>
		<th>주소</th>
		<th colspan="5"><input class="s_input" name="adres" id="adres" type="text"  size="25" title="" value="<c:out value="${resultInfo.adres}"/>" maxlength="20" style="width:100%;" readonly="readonly"></th>	
		<th>메모</th>
		<th colspan="3"><input class="s_input" name="memo" type="text"  size="25" title="" value="<c:out value="${resultInfo.memo}"/>" maxlength="20" style="width:100%;" readonly="readonly"></th>
	</tr>
	</c:forEach>
		</c:otherwise>
	</c:choose>	
...
<table class="board_list" summary="" id="prdlstList">
	...
	<tr style='height:26px;'>
		<th><input type='hidden' name='prdlstSubSn' value='<c:out value="${resultInfo.prdlstSubSn}"/>'><input type='hidden' name='prdlstDirectYn' value='<c:out value="${resultInfo.prdlstDirectYn}"/>'><input class='se_input' name='prdlstJhno' type='text'  size='25' title='' value='<c:out value="${resultInfo.prdlstJhno}"/>'  maxlength='20' style='width:100%;' <c:if test="${resultInfo.prdlstDirectYn == 'N'}"> readonly="readonly"</c:if>></th>
		...
		<th><input class='s_input' name='prdlstModel' type='text'  size='25' title='' value='<c:out value="${resultInfo.prdlstModel}"/>'  maxlength='20' style='width:100%;' readonly="readonly"></th>
		<th><input class='s_input' name='prdlstPart' type='text'  size='25' title='' value='<c:out value="${resultInfo.prdlstPart}"/>'  maxlength='20' style='width:100%;' readonly="readonly"></th>
		<th><input class='s_input' name='prdlstShim' type='text'  size='25' title='' value='<c:out value="${resultInfo.prdlstShim}"/>'  maxlength='20' style='width:100%;' readonly="readonly"></th>
		<th><input class='n_input' name='prdlstUntpc' id='prdlstUntpc' type='text'  size='25' title='' value='<c:out value="${resultInfo.prdlstUntpc}"/>'  maxlength='20' style='width:100%;' readonly="readonly"></th>
		<th><input class='nd_input' name='prdlstProfitRt' type='text'  size='25' title='' value='<c:out value="${resultInfo.prdlstProfitRt}"/>'  maxlength='20' style='width:100%;' readonly="readonly"></th>
		<th><input class='n_input' name='prdlstQy' type='text'  size='25' title='' value='<c:out value="${resultInfo.prdlstQy}"/>'  maxlength='20' style='width:100%;' readonly="readonly"></th>
		<th><input class='n_input' name='prdlstAmount' type='text'  size='25' title='' value='<c:out value="${resultInfo.prdlstAmount}"/>'  maxlength='20' style='width:100%;' readonly="readonly"></th>
	</tr>
	</c:forEach>
	</tbody>
	<tfoot>
...
<table class="board_list" summary="">
	...
	<tr>
		<th>메모</th>			
		<th><pre><textarea style="width:100%; height: 100px;" name="estmtMemo" readonly="readonly"><c:out value="${estmtManageVO.estmtMemo}"/></textarea></pre></th>	
	</tr>
	</thead>
</table>
...
<div class="center_box" style="padding-top:20px;">
	<div class="btn" style="width:190px;">
		<input type="button" class="s_submit" id="btn_excel" value="엑셀양식" title="엑셀양식 다운로드" />
       	<input type="button" class="s_submit" id="btn_edit" value="수정(이동)" title="수정페이지 이동" />
...
<c:import url="/EgovModal.do" charEncoding="utf-8">
	<c:param name="scriptYn" value="N" />
	<c:param name="modalName" value="egovModal3" />
</c:import>
